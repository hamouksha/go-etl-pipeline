...
import (
	...
	"github.com/hamouksha/fast-etl/internel/config"
)
...
func NewCSVReader(source *config.Source, cfgFields []config.Field) (*CSVReader, error) {

	return &CSVReader{source.Location, rune(source.Delimeter), cfgFields}, nil
}
...
func (c *CSVReader) Extract() (<-chan Row, <-chan error, error) {
	...
	if err != nil {
		//errchan <- fmt.Errorf("failed to open file : %v \n", err)
